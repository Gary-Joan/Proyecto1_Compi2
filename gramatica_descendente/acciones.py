...
class acciones ():
 ...
 #-----------------------------------------------------------------------------SENTENCIA IF------------------------------------
    def acciones_if(self, Raiz,consola):
        result = None
        consola1=consola
        if(Raiz!=None):
            ...
            self.error+='Error en la sentencia if\n'
        return result
 #-----------------------------------------------------------------------------COMANDO GOTO------------------------------------
 ...
 #-----------------------------------------------------------------------------EXPRESIONES logicas----------------------------
    ...
    def operaciones_logicas(self, izq,der,op):
        ...
        if (izq!=None) and (der!=None):
            ...
            self.error+="Error En la Expresion Logica\n"
            result=None
        return result
                ...
                if izq=='int':
                    if Raiz.hijos[1].produccion == 'var':
                        ...
                        if result!=None:
                            ...
                            self.error+="Error en la conversion de la variable "+nombre+"\n"
                    else:
                        return result
                elif izq == 'float':
                    if Raiz.hijos[1].produccion == 'var':
                        ...
                        if result!=None:
                            if result.tipo == 'entero':
                                result.valor=str(float(result.valor))
                                result.tipo = 'decimal'
                                return result
                            elif result.tipo == 'cadena':
                                if len(result.valor)==1:
            ...
            if id_variable_simbolo !=None:
                ...
                for acceso in arbol_param_acceso.hijos:
                    ...
                    try:
                        ...
                        self.error+="Error no se encuentra el indice "+ambito.valor+ "\n"
                    if (ambito != None):
